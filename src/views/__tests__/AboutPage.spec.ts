...
describe('About view', () => {
  it('describes the project and links to its source repository', async () => {
    ...
    expect(wrapper.text()).toContain('Elemental icons by Austen Cloud')
    expect(wrapper.text()).toContain('same icons TKA renders on its pictographs')
    expect(wrapper.get('a[href="https://tkaflowarts.com"]').text()).toBe('The Kinetic Alphabet')
    expect(wrapper.findAll('.detail-marker')).toHaveLength(5)
